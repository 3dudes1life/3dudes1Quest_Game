...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8M:',e.error||e.message);
...
class Adventure{
  ...
  reset(startingDude=0){
    ...
    this.zoey={x:105,y:FLOOR-39,w:54,h:38,active:false,cooldown:0,heart:0};
  ...
  updatePets(dt,t){
    ...
    // Zoey protects the team when health is dangerously low.
    if(this.state.health<=1&&this.zoey.cooldown<=0&&!this.state.bossDefeated){
      ...
      this.showPetMoment('Zoey to the rescue!','Health restored and the trio protected.','❤️');
    ...
    if(this.zoey.active){
      ...
      if(this.state.health>1&&this.zoey.heart<=0)this.zoey.active=false;
    }else{
      this.zoey.x+=(Math.max(55,this.player.x-165)-this.zoey.x)*.035*dt;
      this.zoey.y=FLOOR-40;
    }

    // Rare background moment where both dogs run together.
    this.petDuoTimer-=dt;
    if(this.petDuoTimer<=0){
      this.petDuoTimer=520+Math.random()*420;
      this.zoey.active=true;
      this.zoey.heart=105;
      this.showPetMoment('The beagles are on the move!','Rigsby and Zoey are scouting ahead.','🐾');
    }
  }
  ...
  showBossDialogue(stage){
    ...
    const lines={
      1:['QUEEN OF BEIGE','This neighborhood has standards.','Will','So do rainbows.'],
      2:['QUEEN OF BEIGE','Why must everything be so colorful?','Daniel','Because beige is not a personality.'],
      3:['QUEEN OF BEIGE','You cannot defeat the HOA!','Caleb','Good thing I brought cookies.']
    };
  ...
  defeatBoss(){this.boss.alive=false;this.boss.active=false;this.state.bossDefeated=true;this.portal.open=true;this.worldPulse=45;this.haptic([45,40,70,45,110]);this.shake=22;this.state.objective={title:'Enter the Lake Tahoe Portal',detail:'Southern California is restored. The next villain awaits in Adventure 2.'};this.burst(this.boss.x+60,this.boss.y+70,'#ffe66d',100);showAchievement('Queen of Beige Defeated','👑');this.ui.flash('The Queen of Beige has been defeated!')}
  ...
  drawCinematicLife(t){
    ...
    // Draw both beagles as true recurring companions.
    this.drawBeagle(this.rigsby.x,this.rigsby.y,t,'rigsby',this.rigsby.x>this.player.x);
    if(this.zoey.active||Math.abs(this.zoey.x-this.player.x)<430){
  ...
  save(){return {version:'1.0.8M',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>e.alive),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
  applySave(s){if(!s)return;this.player.x=clamp(s.player?.x||150,0,WORLD-50);this.player.y=s.player?.y||FLOOR-72;Object.assign(this.state,s.state||{},{paused:false});s.cards?.forEach((v,i)=>this.cards[i].taken=v);s.beacons?.forEach((v,i)=>this.beacons[i].on=v);s.enemies?.forEach((v,i)=>this.enemies[i].alive=v);s.secrets?.forEach((v,i)=>{if(this.secretSites[i])this.secretSites[i].found=v});this.secretsFound=this.secretSites.filter(site=>site.found).length;if(s.boss){this.boss.hp=s.boss.hp;this.boss.alive=s.boss.alive;this.boss.phase=s.boss.phase||1;this.state.bossDefeated=!s.boss.alive;this.portal.open=!s.boss.alive}this.camera=clamp(this.player.x-350,0,WORLD-W)}
}
...
const SAVE='3dudes1quest-save-108m';
const LEGACY_SAVES=['3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j','3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8M'}))}catch{}
        return legacy;
      }
    }
