...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8R:',e.error||e.message);
...
class Adventure{
  ...
  defeatBoss(){
    ...
    this.input.releaseAll?.();
    this.worldPulse=90;this.shake=28;this.cameraZoom=.92;
  ...
  updateFinale(dt,t){
    ...
    if(f.stage===0&&f.timer>78){
      ...
      f.stage=6;f.timer=0;f.active=false;f.complete=true;
  ...
  save(){return {version:'1.0.8R',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>({alive:e.alive,hp:e.hp,signHp:e.signHp,signUp:e.signUp})),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
const SAVE='3dudes1quest-save-108r';
const LEGACY_SAVES=['3dudes1quest-save-108q','3dudes1quest-save-108p','3dudes1quest-save-108o','3dudes1quest-save-108n','3dudes1quest-save-108m','3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j','3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8R'}))}catch{}
        return legacy;
      }
    }
