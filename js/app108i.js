...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8I:',e.error||e.message);
...
class Adventure{
  ...
  reset(startingDude=0){
    this.state={health:4,cards:0,beacons:0,dude:clamp(startingDude,0,2),triangle:0,paused:false,bossDefeated:false,objective:{title:ZONES[0].objective,detail:ZONES[0].detail}};
  ...
  collect(){
    for(const c of this.cards){if(!c.taken&&overlap(this.player,c)){c.taken=true;this.state.cards++;this.state.triangle=clamp(this.state.triangle+12,0,100);this.burst(c.x,c.y,'#ffe66d',30);showCollect(`🌈 Gay Card ${this.state.cards}/6`)}}
    for(const b of this.beacons){if(!b.on&&overlap(this.player,b)){b.on=true;this.state.beacons++;this.state.triangle=clamp(this.state.triangle+18,0,100);this.burst(b.x,b.y,'#3ce7d2',42);showAchievement(`Prism Beacon ${this.state.beacons}/3`,'🔺')}}
  }
  ...
  save(){return {version:'1.0.8I',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>e.alive),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
function showAchievement(title,icon='🏆'){
  const stack=qs('achievementStack');
  const card=document.createElement('div');
  card.className='achievementCard';
  card.innerHTML=`<div class="achievementIcon">${icon}</div><div><div class="achievementLabel">ACHIEVEMENT</div><div class="achievementTitle">${title}</div></div>`;
  stack.appendChild(card);
  setTimeout(()=>card.remove(),matchMedia('(pointer:coarse)').matches?3000:4200);
}
function showCollect(text){
  const t=qs('collectibleToast');
  t.textContent=text;
  t.classList.remove('isVisible');
  requestAnimationFrame(()=>t.classList.add('isVisible'));
  clearTimeout(showCollect.t);
  showCollect.t=setTimeout(()=>t.classList.remove('isVisible'),1450);
}
...
const SAVE='3dudes1quest-save-108i';
const LEGACY_SAVE='3dudes1quest-save-108h';
function readSave(){
  try{
    ...
    if(legacy){
      localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8I'}));
      return legacy;
    }
    return null;
  }catch{return null}
...
qs('pauseBtn').onclick=()=>game.togglePause();
qs('journalBtn').onclick=()=>game.togglePause();
qs('journalClose').onclick=()=>game.togglePause();
qs('switchBtn').onclick=()=>game.switchDude();
...
qs('saveBtn').onclick=()=>{localStorage.setItem(SAVE,JSON.stringify(game.save()));qs('saveToast').classList.add('show');setTimeout(()=>qs('saveToast').classList.remove('show'),1200);refresh()};
...
function installMobileExperience(){
  const gate=qs('mobileOrientationGate');
  const portraitAnyway=qs('portraitAnywayBtn');
  if(!gate||!portraitAnyway)return;

  let portraitOverride=sessionStorage.getItem('quest-portrait-override')==='1';
  const isTouch=()=>matchMedia('(hover:none), (pointer:coarse)').matches;
  const isPortrait=()=>innerHeight>innerWidth;

  const update=()=>{
    const shouldShow=isTouch()&&isPortrait()&&!portraitOverride&&qs('gameScreen')?.classList.contains('active');
    gate.classList.toggle('hidden',!shouldShow);
    document.body.classList.toggle('mobilePortraitPlaying',isTouch()&&isPortrait()&&portraitOverride);
    document.body.classList.toggle('mobileLandscapePlaying',isTouch()&&!isPortrait());
    if(shouldShow){
      window.__questGame?.releaseInput?.();
    }
  };

  portraitAnyway.addEventListener('click',()=>{
    portraitOverride=true;
    sessionStorage.setItem('quest-portrait-override','1');
    update();
    qs('gameCanvas')?.focus({preventScroll:true});
  });

  addEventListener('resize',update,{passive:true});
  addEventListener('orientationchange',()=>setTimeout(update,120),{passive:true});
  document.addEventListener('visibilitychange',()=>{
    if(document.hidden)window.__questGame?.releaseInput?.();
    update();
  });

  const observer=new MutationObserver(update);
  observer.observe(qs('gameScreen'),{attributes:true,attributeFilter:['class']});
  update();
}
installMobileExperience();

refresh();setInterval(()=>{if(game.running&&!game.state.paused)localStorage.setItem(SAVE,JSON.stringify(game.save()))},30000);
