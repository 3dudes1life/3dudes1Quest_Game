...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.9:',e.error||e.message);
...
class Adventure{
  ...
  start(save,startingDude=0){
    ...
    if(save){
      ...
      this.ui.flash(`${dude.name} ready • ${dude.power}`);
  ...
  update(dt,t){
    ...
    if(this.worldPulse>0)this.worldPulse-=dt;
    if(this.switchBurst>0)this.switchBurst-=dt;if(this.danielCast>0)this.danielCast-=dt;if(this.bossHitFlash>0)this.bossHitFlash-=dt;if(this.arenaPulse>0)this.arenaPulse-=dt;if(this.combatFlash>0)this.combatFlash-=dt;if(this.ultimateFlash>0)this.ultimateFlash-=dt;if(this.comboPulse>0)this.comboPulse-=dt;
  ...
  updateCookieBombs(dt){
    for(const b of this.cookieBombs){
      ...
      b.trail=b.trail||[];b.trail.push({x:b.x+b.w/2,y:b.y+b.h/2});if(b.trail.length>12)b.trail.shift();b.x+=b.vx*dt;b.y+=b.vy*dt;b.vy+=(b.gravity??.28)*dt;b.life-=dt;
      let target=null;
  ...
  updateFinale(dt,t){
    const f=this.finale,previousStage=f.stage;f.timer+=dt;
    ...
    if(f.stage!==previousStage)persistQuest('finale-stage');
    this.ui.hud(this);
  }

  checkPortal(){
    const ready=this.portal.open&&this.state.bossDefeated&&this.state.zoeyUnlocked&&!this.finale.active;
    if(ready&&overlap(this.player,this.portal)&&!this.finished){
      this.finished=true;
      this.state.adventureComplete=true;
      this.running=false;
      persistQuest('adventure-complete');
      setTimeout(()=>this.onComplete(this.state),500);
    }
  }
  draw(t){
    const c=this.ctx;
  ...
  drawCinematicLife(t){
    ...
    // Character-specific switch cue. Daniel intentionally has no body-centered effect.
    if(this.switchBurst>0&&this.state.dude!==1){
      const progress=clamp(this.switchBurst/18,0,1),radius=(1-progress)*48+12;
      ...
      if(this.state.dude===0){
        ...
        c.strokeStyle='#ff9a3c';c.lineWidth=3;c.setLineDash([5,7]);c.beginPath();c.arc(this.player.x+22,this.player.y+55,radius*.78,0,Math.PI*2);c.stroke();c.setLineDash([]);
      }
      c.restore();
    }
  }
  ...
  drawHeroShots(t){
    ...
    // A single clean heart forms at Daniel's hands—no particle cloud behind him.
    if(this.danielCast>0&&this.state.dude===1){
      const facing=this.player.facing||1;
      const progress=clamp(this.danielCast/12,0,1);
      const hx=this.player.x+(facing>0?57:-12);
      const hy=this.player.y+34;
      c.save();
      c.globalAlpha=progress;
      c.translate(hx,hy);
      c.scale(.78+(.22*(1-progress)),.78+(.22*(1-progress)));
      c.shadowColor='#ff79bd';c.shadowBlur=14;
      c.fillStyle='#ff79bd';heartPath(0,0,13);c.fill();
  ...
  drawCookieBombs(t){
    const c=this.ctx;
    for(const b of this.cookieBombs){
      if(!b.stuck&&b.trail?.length){
        c.save();
        for(let i=0;i<b.trail.length;i++){
          const p=b.trail[i],alpha=(i+1)/b.trail.length;
          c.globalAlpha=alpha*.42;
          c.fillStyle=i%2?'#ffe66d':'#b76a35';
          c.beginPath();c.arc(p.x,p.y,1.5+alpha*2.4,0,Math.PI*2);c.fill();
        }
        c.restore();
      }
  ...
  updatePowerButton(){
    ...
    // Keep the mobile control universal: character emoji + POWER.
  ...
  switchDude(i=null){
    ...
    this.haptic(10);this.sound('switch');
    this.heroTheme=DUDES[this.state.dude].color;
    ...
    if(this.switchCombo.heroes.length>=3){
      ...
      // Screen pulse only: no character-centered glitter burst.
      this.combatFlash=Math.max(this.combatFlash,34);
    ...
    const line=lines[this.state.dude][Math.floor(Math.random()*lines[this.state.dude].length)];this.ui.flash(line);this.powerFlash=this.state.dude===1?0:6;
    // Character-specific arrival effects. Daniel has no body-centered visual effect.
  ...
  save(){
    return {
      ...
      cards:this.cards.map(card=>Boolean(card.taken)),
      beacons:this.beacons.map(beacon=>Boolean(beacon.on)),
      ...
      secrets:this.secretSites.map(site=>Boolean(site.found)),
      boss:{
        hp:this.boss.hp,
        alive:Boolean(this.boss.alive),
        active:Boolean(this.boss.active),
        phase:this.boss.phase,
        summoned:Boolean(this.boss.summoned)
      },
      finale:{
        active:Boolean(this.finale.active),
        stage:this.finale.stage,
        timer:this.finale.timer,
        complete:Boolean(this.finale.complete),
        portalReady:Boolean(this.finale.portalReady)
      },
      portal:{open:Boolean(this.portal.open)},
      cityHallDamage:this.cityHallDamage,
  ...
  applySave(s){
    ...
    const savedState=s.state&&typeof s.state==='object'?s.state:{};
    Object.assign(this.state,savedState,{paused:false});

    this.state.health=clamp(Math.round(finite(this.state.health,4)),1,4);
    ...
    this.player.facing=finite(s.player?.facing,1)<0?-1:1;
    this.player.vx=0;
    this.player.vy=0;
    this.player.inv=0;
    ...
    if(Array.isArray(s.cards)){
      s.cards.forEach((value,index)=>{
        if(this.cards[index])this.cards[index].taken=Boolean(value);
      });
    }
    if(Array.isArray(s.beacons)){
      s.beacons.forEach((value,index)=>{
        if(this.beacons[index])this.beacons[index].on=Boolean(value);
      });
    }

    this.state.cards=this.cards.filter(card=>card.taken).length;
    this.state.beacons=this.beacons.filter(beacon=>beacon.on).length;

    if(Array.isArray(s.enemies)){
      s.enemies.forEach((value,index)=>{
        ...
        enemy.alive=value.alive??enemy.alive;
        enemy.hp=finite(value.hp,enemy.hp);
        if('signHp' in value)enemy.signHp=finite(value.signHp,enemy.signHp??1);
        if('signUp' in value)enemy.signUp=Boolean(value.signUp);
      });
    }

    if(Array.isArray(s.secrets)){
      s.secrets.forEach((value,index)=>{
        if(this.secretSites[index])this.secretSites[index].found=Boolean(value);
      });
    }
    this.secretsFound=this.secretSites.filter(site=>site.found).length;

    const savedBoss=s.boss&&typeof s.boss==='object'?s.boss:{};
    this.boss.hp=clamp(finite(savedBoss.hp,this.boss.maxHp),0,this.boss.maxHp);
    this.boss.alive=savedBoss.alive!==false&&this.boss.hp>0;
    const healthPhase=this.boss.hp<=4?3:this.boss.hp<=7?2:1;
    this.boss.phase=clamp(Math.max(healthPhase,Math.round(finite(savedBoss.phase,1))),1,3);
    this.boss.active=Boolean(savedBoss.active&&this.boss.alive);
    this.boss.summoned=Boolean(savedBoss.summoned);
    this.bossIntroShown=this.boss.active;
    this.lastBossPhase=this.boss.phase;

    const defeated=Boolean(this.state.bossDefeated||!this.boss.alive);
    this.state.bossDefeated=defeated;
    this.finished=false;
    ...
    if(defeated){
      ...
      const savedFinale=s.finale&&typeof s.finale==='object'?s.finale:{};
      const savedStage=clamp(Math.round(finite(savedFinale.stage,6)),0,6);
      const interrupted=Boolean(savedFinale.active)&&savedStage<6;
      ...
      if(interrupted){
        this.finale={
          ...
          timer:0,
          complete:false,
          portalReady:savedStage>=5
        };
        this.setInputMode('OVERLAY');
        this.player.x=8420;
        this.player.y=FLOOR-this.player.h;
        this.portal.open=savedStage>=5;

        this.state.zoeyUnlocked=savedStage>=2||Boolean(savedState.zoeyUnlocked);
        this.zoey.active=this.state.zoeyUnlocked;
        this.rigsby.x=savedStage>=2?8460:8425;
        this.zoey.x=savedStage>=2?8660:8665;
        this.zoey.y=FLOOR-40;

        if(savedStage>=4){
          this.celebrationNPCs=Array.from({length:12},(_,index)=>({
            x:8070+index*62,
            y:FLOOR-42,
            bob:0,
            color:['#ff4fb8','#3ce7d2','#ffe66d','#8d5cff','#ff9a3c'][index%5]
          }));
        }
      }else{
        this.finale={
        ...
        this.rigsby.x=this.player.x-128*this.player.facing;
        this.zoey.x=this.player.x-72*this.player.facing;
        this.zoey.y=FLOOR-40;
      }

      this.cityHallDamage=1;
      this.cityHallDamageStage=3;
    }else{
      this.state.bossDefeated=false;
      this.state.zoeyUnlocked=false;
      this.zoey.active=false;
      this.portal.open=false;
      this.finale={active:false,stage:0,timer:0,complete:false,portalReady:false};
      this.cityHallDamage=clamp(
        finite(s.cityHallDamage,1-this.boss.hp/this.boss.maxHp),
        0,
        1
      );
      this.cityHallDamageStage=this.cityHallDamage>=.75?3:this.cityHallDamage>=.45?2:this.cityHallDamage>=.18?1:0;
      this.setInputMode('PLAYING');
    }
    ...
    this.zoneSeen=new Set(ZONES.filter(zone=>this.player.x>=zone.start).map(zone=>zone.id));
    this.zoneSeen.add('home');
    this.camera=clamp(this.player.x-350,0,WORLD-W);
    this.cameraTarget=this.camera;
    this.updatePowerButton();

    if(this.zone().id!=='la'){
      const currentZone=this.zone();
      this.state.objective={title:currentZone.objective,detail:currentZone.detail};
    }else{
      this.updateObjective();
    }
  }
...
const LEGACY_SAVES=[
  ...
  '3dudes1quest-save-108u','3dudes1quest-save-108t','3dudes1quest-save-108s',
  '3dudes1quest-save-108r','3dudes1quest-save-108q','3dudes1quest-save-108p',
  '3dudes1quest-save-108o','3dudes1quest-save-108n','3dudes1quest-save-108m',
  '3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j',
  '3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'
];
...
function readSave(){
  const current=parseStoredSave(SAVE);
  if(current)return current;

  for(const key of LEGACY_SAVES){
    const legacy=parseStoredSave(key);
    if(!legacy)continue;
    const migrated=migrateSave(legacy);
    try{localStorage.setItem(SAVE,JSON.stringify(migrated))}catch(error){
      console.warn('Unable to migrate quest save:',error);
    }
    return migrated;
  }
  return null;
...
function persistQuest(reason='auto'){
  ...
  try{
    ...
    console.warn('Unable to save quest:',error);
    return false;
  }
}
...
function refresh(){
  const save=readSave();
  qs('continueBtn').classList.toggle('hidden',!save);
  if(!save){
    qs('saveStatus').textContent='No saved quest yet.';
    return;
  }
  const timestamp=Date.parse(save.savedAt||'');
  const label=Number.isFinite(timestamp)?new Date(timestamp).toLocaleString():'Recently';
  qs('saveStatus').textContent=`Adventure saved • ${label}`;
}
