...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8Q:',e.error||e.message);
...
function createInput(switcher){
  ...
  addEventListener('keydown',event=>{
    ...
    const gameplayCode=['ArrowLeft','ArrowRight','ArrowUp','KeyA','KeyD','KeyW','KeyX','KeyF','Digit1','Digit2','Digit3'].includes(code);
...
class Adventure{
  ...
  update(dt,t){
    ...
    if(this.switchBurst>0)this.switchBurst-=dt;if(this.bossHitFlash>0)this.bossHitFlash-=dt;if(this.arenaPulse>0)this.arenaPulse-=dt;if(this.combatFlash>0)this.combatFlash-=dt;
  ...
  save(){return {version:'1.0.8Q',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>({alive:e.alive,hp:e.hp,signHp:e.signHp,signUp:e.signUp})),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
const SAVE='3dudes1quest-save-108q';
const LEGACY_SAVES=['3dudes1quest-save-108p','3dudes1quest-save-108o','3dudes1quest-save-108n','3dudes1quest-save-108m','3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j','3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8Q'}))}catch{}
        return legacy;
      }
    }
