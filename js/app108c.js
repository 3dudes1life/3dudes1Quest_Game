...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8C:',e.error||e.message);
...
class Adventure{
  ...
  reset(){
    ...
    this.bossIntroShown=false;this.boss.phase=1;this.boss.summoned=false;this.ui.hud(this);
  }
  ...
  updateBoss(dt,t){
    ...
    if(this.player.x>arenaX&&prereq&&this.boss.alive){
      ...
      if(!this.bossIntroShown){this.bossIntroShown=true;showBossIntro()}
    }
    if(!this.boss.active||!this.boss.alive)return;
  ...
  drawBoss(t){if(!this.boss.alive)return;const c=this.ctx,b=this.boss,phase=this.boss.phase;c.save();const aura=phase===1?'#d8c3a6':phase===2?'#ff4fb8':'#ffe66d';c.shadowColor=this.boss.active?aura:'transparent';c.shadowBlur=this.boss.active?42:0;c.fillStyle='#d3c3ad';c.beginPath();c.roundRect(b.x,b.y,b.w,b.h,26);c.fill();c.fillStyle=phase===1?'#aa9986':phase===2?'#8d5cff':'#ff4fb8';c.beginPath();c.moveTo(b.x+15,b.y+55);c.lineTo(b.x-18,b.y+145);c.lineTo(b.x+138,b.y+145);c.lineTo(b.x+105,b.y+55);c.fill();c.fillStyle='#705554';c.beginPath();c.arc(b.x+60,b.y+38,35,0,Math.PI*2);c.fill();c.fillStyle='#f2e5d2';c.beginPath();c.moveTo(b.x+16,b.y+2);c.lineTo(b.x+35,b.y-42);c.lineTo(b.x+60,b.y-10);c.lineTo(b.x+85,b.y-42);c.lineTo(b.x+105,b.y+2);c.fill();c.fillStyle=phase===3?'#ff284f':'#fff';c.fillRect(b.x+37,b.y+30,12,8);c.fillRect(b.x+72,b.y+30,12,8);this.text('QUEEN',b.x+60,b.y+92,17,'#514643','center');this.text('OF BEIGE',b.x+60,b.y+117,14,'#514643','center');this.text(`PHASE ${phase}`,b.x+60,b.y+140,11,aura,'center');c.restore()}
  ...
  save(){return {version:'1.0.8C',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>e.alive),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
function showBossIntro(){const el=qs('sceneCurtain');if(!el)return;el.querySelector('.sceneCurtainTitle').innerHTML='QUEEN OF BEIGE<br><span style="font-size:.32em;color:#ffe66d">FINAL BOSS</span>';el.classList.add('isActive');setTimeout(()=>el.classList.remove('isActive'),1400)}
...
const SAVE='3dudes1quest-save-108c';
