...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8S:',e.error||e.message);
...
class Adventure{
  ...
  update(dt,t){
    ...
    if(this.worldPulse>0)this.worldPulse-=dt;
    if(this.switchBurst>0)this.switchBurst-=dt;if(this.bossHitFlash>0)this.bossHitFlash-=dt;if(this.arenaPulse>0)this.arenaPulse-=dt;if(this.combatFlash>0)this.combatFlash-=dt;if(this.ultimateFlash>0)this.ultimateFlash-=dt;
  ...
  damageEnemy(e,amount=1,source='rainbow'){
    ...
    if(e.signUp){
      ...
      if(e.signHp<=0){
        // Only damage the protester with attack power that overflowed the sign.
        // A standard Rainbow Toss breaks a one-hit sign without also defeating its holder.
        ...
        remaining=0;
        this.burst(e.x+e.w/2,e.y-20,'#d8c3a6',24);
      }
    }
    ...
    if(e.hp<=0){
      ...
      if(e.type==='protester'||e.type==='leadProtester'){
        ...
        this.enemyDefeated(e,source==='cookie'?'#ff9a3c':'#ff4fb8');
        ...
        this.enemyDefeated(e,source==='cookie'?'#ff9a3c':'#ff4fb8');
        this.ui.flash('HOA Bot rainbowed!');
      }
    }else{
      this.burst(e.x+e.w/2,e.y+e.h/2,'#ff4fb8',18);
    }
  }
  ...
  updateHeroShots(dt){
    for(const s of this.heroShots){
      ...
      for(const e of this.enemies)if(e.alive&&overlap(s,e)){
        s.life=0;this.damageEnemy(e,1,'rainbow');break;
      }
  ...
  explodeCookie(x,y){
    ...
    for(const e of this.enemies)if(e.alive&&Math.hypot(e.x-x,e.y-y)<220)this.damageEnemy(e,2,'cookie')
  ...
  defeatBoss(){
    ...
    for(let i=0;i<70;i++)this.cityHallDebris.push({x:7700+Math.random()*520,y:270+Math.random()*220,vx:(Math.random()-.5)*8,vy:-2-Math.random()*6,spin:(Math.random()-.5)*.22,r:0,life:110+Math.random()*120,size:4+Math.random()*13,color:i%3===0?'#ff4fb8':i%3===1?'#d8c3a6':'#ffe66d'});
  ...
  drawEnemies(t){
    ...
    for(const e of this.enemies){
      ...
      c.fillStyle='#ff4fb8';c.fillRect(e.x+11,e.y+16,4,4);c.fillRect(e.x+37,e.y+16,4,4);
    }
  }
  drawCaptiveZoey(t){
  ...
  drawBossArenaFX(t){
    ...
    // Structural damage: City Hall visibly cracks, sheds masonry and partially collapses.
    if(this.cityHallDamage>0){
      const damage=this.cityHallDamage;
      c.globalCompositeOperation='source-over';
      c.save();

      const cracks=[
        [[7485,304],[7522,350],[7505,398],[7548,444],[7528,505]],
        [[7720,322],[7685,366],[7718,408],[7690,466],[7732,520]],
        [[7965,310],[8010,356],[7988,401],[8034,452],[8002,512]],
        [[8185,330],[8140,374],[8170,423],[8138,478],[8188,530]]
      ];
      const visible=Math.max(1,Math.ceil(damage*cracks.length));
      c.lineCap='round';
      for(let i=0;i<visible;i++){
        const pts=cracks[i];
        c.strokeStyle=`rgba(48,37,55,${.35+damage*.55})`;
        c.lineWidth=3+damage*4;
        c.beginPath();
        c.moveTo(pts[0][0],pts[0][1]);
        for(let j=1;j<pts.length;j++)c.lineTo(pts[j][0],pts[j][1]);
        c.stroke();

        c.strokeStyle=`rgba(255,230,109,${.1+damage*.22})`;
        c.lineWidth=1.2;
        c.stroke();
      }

      // Missing chunks increase with each damage stage.
      const chunks=[
        {x:7472,y:294,w:70,h:44,stage:1},
        {x:7902,y:286,w:88,h:52,stage:2},
        {x:8138,y:335,w:96,h:82,stage:3},
        {x:7660,y:426,w:58,h:115,stage:3}
      ];
      for(const chunk of chunks){
        if(this.cityHallDamageStage<chunk.stage)continue;
        c.fillStyle='rgba(31,23,42,.72)';
        c.beginPath();
        c.moveTo(chunk.x,chunk.y);
        c.lineTo(chunk.x+chunk.w*.72,chunk.y+8);
        c.lineTo(chunk.x+chunk.w,chunk.y+chunk.h*.58);
        c.lineTo(chunk.x+chunk.w*.55,chunk.y+chunk.h);
        c.lineTo(chunk.x+8,chunk.y+chunk.h*.82);
        c.closePath();
        c.fill();
      }

      // Roof and façade sag as the Queen approaches defeat.
      if(damage>.45){
        c.strokeStyle=`rgba(51,39,55,${.38+damage*.38})`;
        c.lineWidth=8;
        c.beginPath();
        c.moveTo(7405,447);
        c.quadraticCurveTo(7770,470+damage*38,8230,448);
        c.stroke();
      }
      if(damage>.72){
        const pulse=.5+.5*Math.sin(t*.018);
        c.fillStyle=`rgba(255,79,184,${.08+pulse*.09})`;
        c.fillRect(7390,280,870,285);
      }
      ...
      for(const d of this.cityHallDebris){
        ...
        c.translate(d.x,d.y);
        c.rotate(d.r);
        c.globalAlpha=clamp(d.life/70,0,1);
        c.fillStyle=d.color;
        c.fillRect(-d.size/2,-d.size/2,d.size,d.size*.72);
        c.restore();
      }
      c.restore();
      c.globalAlpha=1;
  ...
  save(){return {version:'1.0.8S',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>({alive:e.alive,hp:e.hp,signHp:e.signHp,signUp:e.signUp})),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},cityHallDamage:this.cityHallDamage,savedAt:new Date().toISOString()}}
...
const SAVE='3dudes1quest-save-108s';
const LEGACY_SAVES=['3dudes1quest-save-108r','3dudes1quest-save-108q','3dudes1quest-save-108p','3dudes1quest-save-108o','3dudes1quest-save-108n','3dudes1quest-save-108m','3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j','3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8S'}))}catch{}
        return legacy;
      }
    }
