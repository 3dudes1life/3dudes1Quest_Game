...
const WORLD_W = 4600;
const FLOOR = 570;
const SAVE_KEY = "3dudes1quest_tahoe_v205";
const RELEASE = "2.0.5v";
...
function loadProgress(){
  try{
    ...
    ui.hudMemories.textContent=state.memoryFound?"1":"0";
...
function reset() {
  ...
  state.coyoteMs=0; state.jumpBufferMs=0; state.landingGrace=0; state.landingSquash=0; state.dogTrail.length=0; state.beetleBursts.length=0;
...
function press(action,value) {
  if (action==="jump" && value && !state.keys.jump) { state.jumpBufferMs=150; jump(); }
...
function bindHold(button,action) {
  ...
  const down=e=>{
    ...
    press(action,true);
    showInputStatus(action==="jump"?"JUMP":action.toUpperCase());
    canvas.focus({preventScroll:true});
  };
  const up=e=>{
    if(!active && action!=="jump") return;
    e.preventDefault();
    active=false;
  ...
  button.addEventListener("mousedown",down,{passive:false});
  button.addEventListener("mouseup",up,{passive:false});
  button.addEventListener("mouseleave",up,{passive:false});
...
function keyAction(e,isDown){
  const key=(e.key||"").toLowerCase();
  const code=e.code||"";
  let handled=true;
  if(key==="arrowleft"||key==="a"||code==="KeyA") press("left",isDown);
  else if(key==="arrowright"||key==="d"||code==="KeyD") press("right",isDown);
  else if(key==="arrowup"||key==="w"||key===" "||key==="j"||code==="Space"||code==="KeyW"||code==="KeyJ") press("jump",isDown);
  else handled=false;
  if(handled){
    e.preventDefault();
    if(isDown && !e.repeat) showInputStatus(key===" "||code==="Space"?"JUMP":(key||code).toUpperCase());
  }
  return handled;
}

document.addEventListener("keydown",e=>{
  if(keyAction(e,true)) return;
  if(e.key==="1")setDude(0);
  if(e.key==="2")setDude(1);
  if(e.key==="3")setDude(2);
  if(e.key==="Escape"){e.preventDefault();togglePause();}
},{capture:true});

document.addEventListener("keyup",e=>keyAction(e,false),{capture:true});

canvas.addEventListener("pointerdown",()=>canvas.focus({preventScroll:true}));
window.addEventListener("blur",()=>{
  state.keys.left=state.keys.right=state.keys.jump=false;
  document.querySelectorAll("[data-action]").forEach(b=>b.classList.remove("pressed"));
});
...
window.__TAHOE_RELEASE__={version:"2.0.5v",name:"World Polish",publicDevShortcuts:false,features:["automatic checkpoint save","resume after refresh","saved beetles and memory","fallen logs and pinecones","wood footbridge","granite boulders","improved Pine Beetle art","beetle defeat bursts","landing squash and jump stretch","running dust","trail-based dog following","Zoey tail wag"]};
})();