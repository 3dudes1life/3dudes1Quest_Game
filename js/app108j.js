...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8J:',e.error||e.message);
...
class Adventure{
  ...
  save(){return {version:'1.0.8J',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>e.alive),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
const SAVE='3dudes1quest-save-108j';
const LEGACY_SAVES=['3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8J'}))}catch{}
        return legacy;
      }
    }
