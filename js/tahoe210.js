...
const WORLD_W = 4600;
const FLOOR = 570;
const SAVE_KEY = "3dudes1quest_tahoe_v210";
const RELEASE = "2.1.0";
...
const state = {
  ...
  exitPortal:{x:4380,y:315,r:0,alpha:0,open:false},
  finaleStage:0,
  finaleTimer:0,
  exitTriggered:false,
  forestRestore:0,
...
const ui = {
  ...
  continueToPortal:document.getElementById("continueToPortal"),
  finaleBanner:document.getElementById("finaleBanner"),
...
function saveProgress(showToast=false){
  try{
    const data={
      ...
      adventureComplete:state.complete
    };
...
function reset() {
  ...
  state.exitPortal.open=false;state.exitPortal.r=0;state.exitPortal.alpha=0;
  state.finaleStage=0;state.finaleTimer=0;state.exitTriggered=false;state.forestRestore=0;
  state.resumeIntro=false;
  state.dudeIndex=0;
  ...
  ui.finaleBanner?.classList.add("hidden");
...
function updateCinematic(now){
  ...
  if(t<2400){state.cameraX=0;ui.cinematicText.textContent=state.resumeIntro?"Returning to your Tahoe checkpoint...":"The portal is opening over the lake...";}
...
function finish() {
  if (state.complete || state.finale) return;
  state.finale=true;
  state.finaleStage=1;
  state.finaleTimer=performance.now();
  state.player.vx=0;
  state.exitPortal.open=false;
  setObjective("The forest is healing","Stay at the summit while color returns to the pines.");
  ui.finaleBanner.classList.remove("hidden");
  ui.finaleBanner.querySelector("small").textContent="MISSION COMPLETE";
  ui.finaleBanner.querySelector("strong").textContent="All Pine Beetles defeated.";
  ui.finaleBanner.querySelector("span").textContent="The forest is beginning to recover.";
  saveProgress(true);
}

function updateFinale(now){
  if(!state.finale || state.complete)return;
  const elapsed=now-state.finaleTimer;
  state.player.vx=0;
  state.forestRestore=Math.min(1,elapsed/2600);
  if(state.finaleStage===1 && elapsed>2600){
    state.finaleStage=2;
    state.exitPortal.open=true;
    state.exitPortal.alpha=1;
    state.exitPortal.r=78;
    setObjective("Enter the Alaska Portal","The path north is open at the summit.");
    ui.finaleBanner.querySelector("small").textContent="FOREST RESTORED";
    ui.finaleBanner.querySelector("strong").textContent="The portal to Alaska is open.";
    ui.finaleBanner.querySelector("span").textContent="Walk into the portal at the summit.";
    for(let i=0;i<120;i++)state.particles.push({
      x:state.exitPortal.x+(Math.random()-.5)*150,
      y:state.exitPortal.y+(Math.random()-.5)*150,
      vx:(Math.random()-.5)*5,vy:(Math.random()-.5)*5,life:90
    });
    setTimeout(()=>ui.finaleBanner.classList.add("hidden"),3200);
  }
}

function checkExitPortal(){
  if(!state.exitPortal.open || state.exitTriggered)return;
  const portalBox={x:state.exitPortal.x-62,y:state.exitPortal.y-85,w:124,h:170};
  if(overlap(state.player,portalBox)){
    state.exitTriggered=true;
    state.complete=true;
    saveProgress(true);
    releaseAllInput();
    document.body.classList.add("portalExit");
    setTimeout(()=>{window.location.href="alaska.html";},850);
  }
}
...
function update(dt) {
  ...
  if (p.x>4300) {
    if(state.beetlesDefeated>=5 && !state.finale) finish();
    else {
      p.x=4260;p.vx=-3;
...
function draw() {
  ...
  if(state.finale) drawFinaleRecovery();
  drawExitPortal();
  drawParticles();
...
function drawFinaleRecovery(){
  if(state.forestRestore<=0)return;
  ctx.save();
  ctx.globalAlpha=.12+state.forestRestore*.14;
  const g=ctx.createLinearGradient(0,0,0,H);
  g.addColorStop(0,"rgba(255,226,116,.1)");
  g.addColorStop(1,"rgba(61,255,161,.52)");
  ctx.fillStyle=g;ctx.fillRect(0,0,W,H);
  ctx.globalAlpha=state.forestRestore;
  for(let i=0;i<35;i++){
    const wx=3350+i*39;
    const x=wx-state.cameraX;
    if(x<-30||x>W+30)continue;
    const y=groundAt(wx)-5;
    ctx.fillStyle=i%3===0?"#ffd65a":i%3===1?"#ff7dbb":"#84e88f";
    ctx.beginPath();ctx.arc(x,y-(i%4)*4,3+(i%2),0,Math.PI*2);ctx.fill();
  }
  ctx.restore();
}
...
function drawExitPortal(){
  ...
  const pulse=1+Math.sin(state.time*.006)*.06;
  ctx.save();ctx.translate(x,y);ctx.scale(pulse,pulse);
  ctx.globalCompositeOperation="lighter";
  ctx.shadowColor="#80e8ff";ctx.shadowBlur=45;
  ctx.strokeStyle="#ff58ba";ctx.lineWidth=15;
  ctx.beginPath();ctx.ellipse(0,0,58,83,0,0,Math.PI*2);ctx.stroke();
  ctx.strokeStyle="#5de4df";ctx.lineWidth=9;
  ctx.beginPath();ctx.ellipse(0,0,42,66,0,0,Math.PI*2);ctx.stroke();
  const g=ctx.createRadialGradient(0,0,5,0,0,48);
  g.addColorStop(0,"rgba(255,255,255,.95)");
  g.addColorStop(.35,"rgba(112,213,255,.75)");
  g.addColorStop(1,"rgba(67,32,137,.2)");
  ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(0,0,37,59,0,0,Math.PI*2);ctx.fill();
  ctx.restore();
  ctx.save();ctx.fillStyle="#fff";ctx.font="1000 15px system-ui";ctx.textAlign="center";
  ctx.shadowColor="#10243a";ctx.shadowBlur=8;ctx.fillText("ALASKA",x,y-102);ctx.restore();
}
...
window.__TAHOE_RELEASE__={version:"2.1.0",name:"Adventure 2 Complete",publicDevShortcuts:false,features:["fixed Enter Portal cinematic crash","fully playable Tahoe golden path","five beetle mission","forest restoration finale","walkable Alaska exit portal","Adventure 2 completion save","Alaska coming soon destination","Level One style world transition"]};
})();