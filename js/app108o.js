...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8O:',e.error||e.message);
...
function createInput(switcher){
  ...
  const gameplayCodes=new Set(['ArrowLeft','ArrowRight','ArrowUp','KeyA','KeyD','KeyW','Space','KeyX','KeyF','Digit1','Digit2','Digit3']);

  const game=()=>window.__questGame;
  const isPlaying=()=>game()?.inputMode==='PLAYING'&&!game()?.state.paused;

  const releaseAll=()=>{
    k.left=k.right=k.jump=k.power=false;
    activePointers.clear();
    document.querySelectorAll('#touchControls button.isPressed').forEach(b=>b.classList.remove('isPressed'));
  };
  ...
  addEventListener('keydown',e=>{
    ...
    if(e.code==='ArrowUp'||e.code==='KeyW')k.jump=true;
    if(e.code==='Space'||e.code==='KeyX'||e.code==='KeyF')k.power=true;
    if(e.code==='Digit1')switcher(0);
    if(e.code==='Digit2')switcher(1);
    if(e.code==='Digit3')switcher(2);
  ...
  addEventListener('keyup',e=>{
    ...
    if(e.code==='Space'||e.code==='KeyX'||e.code==='KeyF')k.power=false;
  },{capture:true,passive:false});
  ...
  document.querySelectorAll('[data-action]').forEach(button=>{
    ...
    const press=event=>{
      ...
      if(!isPlaying())return;
      activePointers.set(event.pointerId,action);k[action]=true;
...
class Adventure{
  ...
  reset(startingDude=0){
    ...
    this.lastBossPhase=1;this.bossHitFlash=0;this.arenaPulse=0;
  ...
  update(dt,t){
    ...
    if(this.switchBurst>0)this.switchBurst-=dt;if(this.bossHitFlash>0)this.bossHitFlash-=dt;if(this.arenaPulse>0)this.arenaPulse-=dt;
  ...
  save(){return {version:'1.0.8O',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>e.alive),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},savedAt:new Date().toISOString()}}
...
const SAVE='3dudes1quest-save-108o';
const LEGACY_SAVES=['3dudes1quest-save-108n','3dudes1quest-save-108m','3dudes1quest-save-108l','3dudes1quest-save-108k','3dudes1quest-save-108j','3dudes1quest-save-108i','3dudes1quest-save-108h','3dudes1quest-save-108g'];
function readSave(){
  try{
    ...
    for(const key of LEGACY_SAVES){
      ...
      if(legacy){
        try{localStorage.setItem(SAVE,JSON.stringify({...legacy,version:'1.0.8O'}))}catch{}
        return legacy;
      }
    }
...
qs('pauseBtn').onclick=()=>{const j=qs('journal');j.classList.remove('hidden');game.state.paused=true;game.setInputMode('PAUSED');game.input.releaseAll?.()};
qs('journalBtn').onclick=()=>{const j=qs('journal');const opening=j.classList.contains('hidden');j.classList.toggle('hidden',!opening);if(opening){game.state.paused=true;game.setInputMode('PAUSED')}else game.resumeGameplay();game.input.releaseAll?.()};
qs('journalClose').onclick=()=>{qs('journal').classList.add('hidden');game.resumeGameplay()};
{
...
qs('resumeBtn').onclick=()=>{qs('journal').classList.add('hidden');game.resumeGameplay()};
