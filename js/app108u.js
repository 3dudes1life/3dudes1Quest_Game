...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8U:',e.error||e.message);
...
const DUDES=[
  {name:'Will',color:'#ff4fb8',accent:'#ffe66d',speed:6.2,jump:14,power:'Rainbow Toss',shortPower:'BLAST',powerIcon:'🌈'},
  {name:'Daniel',color:'#8d5cff',accent:'#3ce7d2',speed:5.7,jump:13.5,power:'Heartfield Burst',shortPower:'HEARTS',powerIcon:'💖'},
  {name:'Caleb',color:'#ff9a3c',accent:'#fff',speed:5.9,jump:13,power:'Cookie Chaos',shortPower:'BOMB',powerIcon:'🍪'}
];
const ZONES=[
...
class Adventure{
  ...
  start(save,startingDude=0){
    ...
    this.updatePowerButton();
    this.ui.flash(`${dude.name} ready • Tap POWER for ${dude.power}`);
  ...
  updateEnemies(dt){
    ...
    for(const e of this.enemies){
      if(!e.alive)continue;
      if(e.heartHeld>0){e.heartHeld-=dt;e.vx*=Math.pow(.82,dt)}
      e.x+=e.vx*dt;
  ...
  damageEnemy(e,amount=1,source='rainbow'){
    ...
    if(e.hp<=0){
      ...
      if(e.type==='protester'||e.type==='leadProtester'){
        ...
        this.enemyDefeated(e,source==='cookie'?'#ff9a3c':source==='heart'?'#3ce7d2':'#ff4fb8');
  ...
  power(t){
    ...
    if(dude===1){
      const facing=this.player.facing||1;
      this.heroShots.push({
        type:'heart',x:this.player.x+(facing>0?44:-20),y:this.player.y+22,w:38,h:34,
        vx:facing*10.2,vy:0,life:132,age:0,trail:[],facing
      });
      this.player.inv=Math.max(this.player.inv,42);
      this.shake=Math.max(this.shake,4);
      this.burst(this.player.x+22,this.player.y+32,'#ff79bd',26);
      this.burst(this.player.x+22,this.player.y+32,'#3ce7d2',22);
      this.ui.flash('Heartfield launched!');return;
    }
  ...
  updateHeroShots(dt){
    for(const s of this.heroShots){
      s.age=(s.age||0)+dt;
      s.trail.push({x:s.x,y:s.y});if(s.trail.length>(s.type==='heart'?15:11))s.trail.shift();
      ...
      if(s.type==='heart'){
        ...
        if(target){
          const targetY=target.y+target.h*.45-s.h/2;
          s.vy+=(targetY-s.y)*.018*dt;
          s.vy=clamp(s.vy,-4.2,4.2);
          s.vx+=Math.sign((target.x+target.w/2)-(s.x+s.w/2))*.08*dt;
          s.vx=clamp(s.vx,-12.5,12.5);
        }
        s.y+=s.vy*dt;
      }
      ...
      for(const e of this.enemies)if(e.alive&&overlap(s,e)){
        s.life=0;hit=true;
        if(s.type==='heart')this.spawnPositiveField(e.x+e.w/2,e.y+e.h/2,e,false);
        else this.damageEnemy(e,1,'rainbow');
        break;
      }
      if(!hit&&this.boss.active&&this.boss.alive&&overlap(s,this.boss)){
        s.life=0;
        if(s.type==='heart')this.spawnPositiveField(this.boss.x+this.boss.w/2,this.boss.y+this.boss.h/2,this.boss,true);
        else this.damageBoss(1,'Rainbow hit!');
      }
    }
  ...
  spawnPositiveField(x,y,target,isBoss=false){
    ...
    if(target&&!isBoss)target.heartHeld=62;
    ...
    for(const f of this.positiveFields){
      ...
      if(!f.detonated&&f.age>=30){
        ...
        if(f.isBoss){
          ...
          this.damageEnemy(f.target,amount,'heart');
        }
        ...
        this.heartCombo++;
        this.ui.flash(f.absorbed?`Heartfield burst • ${f.absorbed} attack${f.absorbed===1?'':'s'} absorbed!`:'Heartfield burst!');
      }
    }
  ...
  switchDude(i=null){
    ...
    const lines=[
      ...
      ['Daniel: Sending love!','Daniel: Heartfield incoming.'],
  ...
  save(){return {version:'1.0.8U',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>({alive:e.alive,hp:e.hp,signHp:e.signHp,signUp:e.signUp})),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},cityHallDamage:this.cityHallDamage,savedAt:new Date().toISOString()}}
