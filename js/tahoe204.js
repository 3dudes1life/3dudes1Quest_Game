...
function reset() {
  ...
  state.coyoteMs=0; state.jumpBufferMs=0; state.landingGrace=0;
...
function drawBeetles(){
  for(const b of state.beetles){
    ...
    ctx.translate(0,bob);
    ctx.fillStyle="#351b16";
    ctx.beginPath();ctx.ellipse(0,0,18,12,0,0,Math.PI*2);ctx.fill();
    ctx.fillStyle="#6d2d24";
    ctx.beginPath();ctx.ellipse(3,-1,11,10,0,0,Math.PI*2);ctx.fill();
...
window.__TAHOE_RELEASE__={version:"2.0.4",name:"Playable Input Fix",publicDevShortcuts:false,features:["Safari keyboard capture","mouse and trackpad hold controls","touch fallback controls","input status confirmation","fully visible desktop controls","improved mallard duck artwork","five-heart forgiving mode","coyote-time jumps","jump buffering"]};
})();