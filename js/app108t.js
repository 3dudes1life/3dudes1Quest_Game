...
window.addEventListener('error',e=>{
  console.error('3Dudes1Quest 1.0.8T:',e.error||e.message);
...
class Adventure{
  ...
  save(){return {version:'1.0.8T',player:{x:this.player.x,y:this.player.y},state:this.state,cards:this.cards.map(c=>c.taken),beacons:this.beacons.map(b=>b.on),enemies:this.enemies.map(e=>({alive:e.alive,hp:e.hp,signHp:e.signHp,signUp:e.signUp})),secrets:this.secretSites.map(s=>s.found),boss:{hp:this.boss.hp,alive:this.boss.alive,phase:this.boss.phase},cityHallDamage:this.cityHallDamage,savedAt:new Date().toISOString()}}
