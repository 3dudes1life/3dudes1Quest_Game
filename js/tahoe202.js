...
function reset() {
  ...
  ui.hudHealth.textContent="3";
  ui.hudBeetles.textContent="0";
  ui.memoryCard.classList.add("hidden");
  ui.trailheadCard.classList.add("hidden");
  ui.missionCard.classList.add("hidden"); ui.cinematicCaption.classList.add("hidden"); ui.photoFrame.classList.add("hidden"); ui.checkpointCard.classList.add("hidden");
  setDude(0);
...
function updateBeetles(dt, oldY){
  ...
  for(const b of state.beetles){
    ...
    if(overlap(state.player,b)){
      const landed=state.player.vy>0 && oldY+state.player.h<=b.y+10;
...
window.__TAHOE_RELEASE__={version:"2.0.2",name:"Trail Climb",features:["five stompable Pine Beetles","three-heart health system","damage knockback","checkpoint respawn","trail completion gate","expanded objectives","granite overlook checkpoint"]};
})();